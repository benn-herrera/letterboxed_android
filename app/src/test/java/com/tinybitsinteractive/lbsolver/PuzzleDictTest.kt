...
class PuzzleDictTest {
    ...
    fun unfilteredSourceConstructorCheck() {
        val testDict = PuzzleDict(StringReader(testWordsUnfiltered).buffered())
    ...
    fun filteredCacheSourceConstructorCheck() {
        val testDict = PuzzleDict(StringReader(testWordsFiltered).buffered()) {
            word -> word.text.first() == 'b'
        }
